...
{ 
	if (IDLE_STATE == CurState())
	{
		...
		memcpy(&tClientInfo,pMsg->content,pMsg->length);
		...
		memcpy(m_achIpAddr,tClientInfo.achIpaddr,sizeof(tClientInfo.achIpaddr));
		memcpy(m_achusername,tClientInfo.achusername,sizeof(tClientInfo.achusername)); 
...
void CCDateInstance::DaemonProcConnectNack(CMessage *const pMsg) //收到ack，连接成功
{
	if (IDLE_STATE == CurState())
	{
		cbShowMsgtoConsole("收到nack反馈，用户名不正确!",OutputMsgtoUser);
		//post(MAKEIID(SERVER_MSGAPP_NO,CInstance::DAEMON),C_S_REQUSERLIST_REQ,NULL,0,m_dwServerNode);//直接获取用户列表
		logprintf(LOG_LVL_KEYSTATUS,"Daemon: recv server connect nack !Connect failed!\n");
	}
	else
	{
		logprintf(LOG_LVL_ERROR,"Daemon: ERROR recv server nack!\n");
	}
}
